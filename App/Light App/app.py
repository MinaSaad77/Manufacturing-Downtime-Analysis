...
# --------------------------------------------------------------------------
# Visual Theme — Industrial Control-Room Styling (Light Mode)
# --------------------------------------------------------------------------
def inject_theme():
    """
    ...
    st.markdown(
        ...
        .stApp {
            background:
                radial-gradient(circle at 15% 0%, rgba(217, 119, 6, 0.05), transparent 40%),
                repeating-linear-gradient(180deg, rgba(0,0,0,0.012) 0px, rgba(0,0,0,0.012) 1px, transparent 1px, transparent 3px),
                var(--bg);
            color: var(--text);
        ...
        h1, h2, h3 { font-family: 'Space Grotesk', sans-serif !important; letter-spacing: -0.01em; color: var(--text); }
        ...
        .status-dot { display: inline-block; width: 8px; height: 8px; border-radius: 50%; background: var(--success); margin-right: 6px; box-shadow: 0 0 8px rgba(15, 157, 99, 0.5); }
        ...
        .verdict-ok { background: rgba(15, 157, 99, 0.06); border-color: rgba(15, 157, 99, 0.3); }
        .verdict-fail { background: rgba(220, 53, 69, 0.06); border-color: rgba(220, 53, 69, 0.3); }
        .verdict-icon { font-size: 1.8rem; line-height: 1; }
        .verdict-title { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 1.15rem; margin-bottom: 0.2rem; color: var(--text); }
        ...
        .prob-row-head { display: flex; justify-content: space-between; font-size: 0.85rem; margin-bottom: 4px; color: var(--text); }
        ...
        /* -----------------------------------------------------------
           Safety-net overrides for Streamlit's NATIVE widgets.
           These aren't our custom HTML, so they need to be forced
           to the light palette too (labels, header bar, alerts,
           dataframe, buttons) in case the .streamlit/config.toml
           theme gets overridden by a deployment's own settings.
        ----------------------------------------------------------- */
        ...
        [data-testid="stSidebar"] .kicker { color: var(--accent) !important; }
        ...
        /* Number input and Text input boxes: force a light box with dark text
           (previously only the text color was forced, leaving dark
           text on a leftover dark box = invisible) */
        ...
        /* Selectbox: same light-box treatment */
        [data-testid="stSelectbox"] div[data-baseweb="select"] > div {
            background: var(--panel) !important;
            color: var(--text) !important;
            border: 1px solid var(--panel-border) !important;
        }
        [data-testid="stSelectbox"] svg { fill: var(--text) !important; }

        /* Dropdown menu (options list) when selectbox is opened */
...
def render_sidebar():
    ...
    -------
    tuple(str, dict, bool)
        Selected machine type, a dict of numeric input values, and whether
    ...
    return machine_type, numeric_values, user_email, predict_clicked
...
def main():
    ...
    # ---------------------- Sidebar Inputs ----------------------
    machine_type, numeric_values, user_email, predict_clicked = render_sidebar()

    # ---------------------- Overview Panel ----------------------
    st.markdown('<div class="section-label">🧾 Current Sensor Readout</div>', unsafe_allow_html=True)

    type_card = readout_card_html("🏭", "Type", f"{machine_type}", TYPE_DESCRIPTIONS[machine_type])
    field_cards = "".join(
        readout_card_html(
    ...
    if predict_clicked:
        ...
        try:
            input_df = build_input_dataframe(machine_type, numeric_values)
